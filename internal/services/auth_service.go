...
func (s *AuthService) Signup(ctx context.Context, req SignupRequest) (*models.User, *models.Session, error) {
	...
	// Create session
	session, err := s.createSession(ctx, user.ID, req.IPAddress, req.UserAgent, req.DeviceID, req.DeviceName)
	if err != nil {
		return nil, nil, err
	}
...
func (s *AuthService) createSession(ctx context.Context, userID uuid.UUID, ipAddress, userAgent, deviceID, deviceName *string) (*models.Session, error) {
	sessionID := uuid.New()
